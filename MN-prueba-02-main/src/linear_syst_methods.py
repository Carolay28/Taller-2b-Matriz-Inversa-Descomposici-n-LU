...
def gauss_jordan(A: np.ndarray) -> np.ndarray:
    """Realiza la elminacion de Gauss-Jordan""

    ## Parameters

    ``A``: matriz del sistema de ecuaciones lineales. Debe ser de tamaño n-by-(n+1), donde n es el número de incógnitas.
    ...
    for i in range(0, n - 1):  # loop por columna
        ...
        # --- Eliminación: loop por fila
        #for j in range(i + 1, n): Eliminacion Guassiana
        for j in range(0 , n):
            if j == i:
                continue 
            m = A[j, i] / A[i, i]
